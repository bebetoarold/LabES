...
void Simulator::run_simulator() {
  ...
  double delta_t_Mseconds = m_params.delta_t * 1000.0;
  ...
  while (T < m_params.simulation_time) {
    ...
    }
    {
      ...
      ///@todo Make sure delay index is within bounds of buffer size
       int jitter = std::experimental::randint(-m_params.jitter, m_params.jitter);
      
      // Calculate delay index
      delay_index = i- std::floor((m_params.delay + jitter) / delta_t_Mseconds);
      //delay_index = i - m_params.delay - jitter;

      
      std::cout << "delayIndex before: " << delay_index << std::endl;
      ...
      /*while (delay_index < 0) {
          delay_index += buffer_size;
          std::cout << "delayIndex during: " << delay_index << std::endl;
      }

      // Make sure delay index is within bounds of buffer size
      delay_index %= buffer_size;
    */
      std::cout << "delayIndex after: " << delay_index << std::endl;
